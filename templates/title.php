...
if( is_single() || is_page() ) {
	...
	if( get_post($id)->post_type == 'sites' ){
		...
		if(empty($tags)) $tags = get_the_title().','.$blog_name;
	...
	if(get_post($id)->post_type =='app'){
		...
		$title = get_post_meta($id, '_seo_title', true)?:(get_post_meta($id, '_app_name', true)?(get_post_meta($id, '_app_name', true).$appinfo['app_version'].($appinfo['app_ad'] ? __('有广告','i_theme') : __('无广告','i_theme')).($appinfo['app_status']=="official"?__('官方版','i_theme'):__('开心版','i_theme')).'-'.$appinfo['app_date']):get_the_title());
	...
	if ($tag != "") {
		$keywords = $tag;
	} elseif (io_get_option('seo_home_keywords', '')) {
		$keywords = _iol(io_get_option('seo_home_keywords', ''));
	}
	if (!empty($excerpt)) {
		$description = $excerpt;
...
if(is_category()){ 
	...
	if($meta && $meta['seo_metakey'])
		...
		$keywords=single_cat_title('', false).','.$blog_name;
	if($meta && $meta['seo_desc'])
		$description=$meta['seo_desc'];
	else
		$description = category_description()?:$option_desc; 
}
if(is_tag() ){ 
	...
	if($meta && $meta['seo_metakey'])
		...
		$keywords=single_tag_title('', false).','.$blog_name;
	if($meta && $meta['seo_desc'])
		$description=$meta['seo_desc'];
	else
		$description=strip_tags(trim(tag_description()))?:$option_desc; 
}
...
<?php if(io_get_option('og_switcher',true)): ?>
